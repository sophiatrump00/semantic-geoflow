...
> **本版本说明**：这是基于 [GEOFlow](https://github.com/yaojingang/GEOFlow) 的二次开发版本，新增了 **AI 语义缓存**功能模块。原项目作者 Yao Jingang（姚金刚），Apache-2.0 许可协议。所有新增功能在相同协议下发布。详见 [CHANGELOG.md](CHANGELOG.md)。
...
| ⚡ AI 语义缓存（新增） | **基于 prompt embedding 向量相似度的智能缓存层**，通过精确匹配 + 语义相似度双层检索，让重复或相似的生成请求直接复用历史结果，降低 30-50% token 消耗，命中响应时间从秒级降至毫秒级。支持 TTL + LRU 失效策略、可配置相似度阈值（默认 0.92）、完整的后台管理界面（查看、统计、清理）。详见 [CHANGELOG.md](CHANGELOG.md) 和 [面试预案](docs/语义缓存-面试预案.md) |
