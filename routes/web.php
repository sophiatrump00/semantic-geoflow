...
use App\Http\Controllers\Admin\AdminSemanticCacheController;
...
Route::middleware(['site.locale', 'site.view_log'])->group(function (): void {
    Route::get('/', fn () => view('landing'))->name('site.home');
...
Route::prefix($adminPrefix)->name('admin.')->middleware(['admin.locale'])->group(function () {
    ...
    Route::middleware(['admin.auth', 'admin.activity'])->group(function () {
        ...
        Route::group([], function () {
            ...
            Route::prefix('semantic-cache')->name('semantic-cache.')->group(function () {
                Route::get('/', [AdminSemanticCacheController::class, 'index'])->name('index');
                Route::get('{id}', [AdminSemanticCacheController::class, 'show'])->name('show');
                Route::post('{id}/delete', [AdminSemanticCacheController::class, 'destroy'])->name('delete');
                Route::post('prune-expired', [AdminSemanticCacheController::class, 'pruneExpired'])->name('prune-expired');
                Route::post('truncate', [AdminSemanticCacheController::class, 'truncate'])->name('truncate');
                Route::get('statistics/data', [AdminSemanticCacheController::class, 'statistics'])->name('statistics');
            });
