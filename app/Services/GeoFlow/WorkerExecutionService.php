...
class WorkerExecutionService
{
    ...
    private function generateContentWithModelSelection(Task $task, string $contentPrompt): array
    {
        ...
        foreach ($this->resolveAiModelCandidates($task) as $candidate) {
            ...
            // 语义缓存查询：先尝试精确匹配或相似度匹配
            $cacheResult = $this->semanticCacheService->get($contentPrompt, (int) $candidate->id);
            if ($cacheResult['hit']) {
                $promptTokens = $this->estimateTokenCount($contentPrompt);
                $responseTokens = $this->estimateTokenCount((string) $cacheResult['content']);
                $this->semanticCacheService->recordHit(
                    (int) $cacheResult['cache_id'],
                    $promptTokens + $responseTokens
                );

                $attempts[] = $this->buildModelAttempt($candidate, 'cache_hit', sprintf(
                    'Semantic cache hit (similarity: %.2f)',
                    $cacheResult['similarity'] ?? 1.0
                ));

                return [
                    'content' => (string) $cacheResult['content'],
                    'model' => $candidate,
                    'attempts' => $attempts,
                    'cache_hit' => true,
     ...
     * 估算文本的 token 数
     *
     * 简化版估算：中文约 1.5 字符/token，英文约 4 字符/token。
     * 用于语义缓存的 token 统计，不需要精确到每个模型的 tokenizer。
     */
