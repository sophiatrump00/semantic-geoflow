...
return [
    ...
    // AI 语义缓存：基于 prompt embedding 相似度检索，降低重复或相似请求的 AI 调用成本
    'semantic_cache_enabled' => filter_var(env('GEOFLOW_SEMANTIC_CACHE_ENABLED', true), FILTER_VALIDATE_BOOLEAN),
    // 相似度阈值（0.0-1.0）：余弦相似度 >= 阈值时视为命中。推荐 0.92（较严格），可调整到 0.88-0.95
    'semantic_cache_similarity_threshold' => max(0.0, min(1.0, (float) env('GEOFLOW_SEMANTIC_CACHE_SIMILARITY_THRESHOLD', 0.92))),
    // 缓存 TTL（秒）：超过此时间的缓存条目将被视为过期。默认 7 天
    'semantic_cache_ttl_seconds' => max(3600, (int) env('GEOFLOW_SEMANTIC_CACHE_TTL_SECONDS', 604800)),
    // 最大缓存条目数（LRU）：超出后删除最久未命中的条目。0 表示不限制
    'semantic_cache_max_entries' => max(0, (int) env('GEOFLOW_SEMANTIC_CACHE_MAX_ENTRIES', 10000)),
